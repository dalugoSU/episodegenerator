...
def get_rec():
    try:
        ...
        for j in search(show_search, tld="co.in", num=1, stop=1,
                        pause=1):  # search method from google search, using show_search variable
        ...
        def GetTitles(episode_tags):  # Pass episode_tags from above
            ...
            episodeTitle = ' '.join(
                [str(elem) for elem in titleSplit])  # Turns list into a string containing episode titles
            return episodeTitle  # returns the episode title

        titles = [GetTitles(tag) for tag in
                  episode_tags]  # for each td.summary element, pull episode title using function which turns it into string
...
def get_link(main_show,
             episode):  # If user selects yes to suggestion, pass show and episode and do google search and open that link
...
bottom_frame_entry = tk.Entry(bottom_frame, bg='light yellow',
                              textvariable=show)  # Entry, textVariable allows to pull input
